...
def predict(model, dataset, device, return_paragraph_predictions=True):
    model.eval()
        ...
        for batch in train_dataloader:
            try:
              ...
              if global_step % val_eval_freq == 0:
                  ...
                  if para_num_total == 0:
                      kloss = loss_sum / num_total
                      
                  else:
                      kloss = loss_sum / para_num_total
                  ...
                  print("STEP: {:7} | TIME: {:4}min | TRAIN LOSS: {:.4f} | VAL LOSS: {:.4f} | VAL ACC(PARA): {:.4f} | VAL ACC(DOC): {:.4f} | VAL AUROC(DOC): {:.4f}".format(
                      global_step, minutes_elapsed, kloss, val_loss, val_para_acc, val_acc, val_auc
                  ))
